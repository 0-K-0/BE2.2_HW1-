...
try{await mongoose.connect(process.env.ConnectionURL) ;
...
const db_disconnect=async()=>{     console.log("//////////////////////////////////")
...
try{
    connectIntoTheDB()
    const viewRestaurant=await RestaurantsModel.findOne({name:restaurantName})
...
const reservationOffer=async()=>{
    try{
const reservableRestaurant=await RestaurantsModel.findOne({reservationsNeeded:true})
...
const restaurantFindByPH_Number=async(number)=>{
    try{
        const theRestaurant=await RestaurantsModel.findOne({phoneNumber:number})
